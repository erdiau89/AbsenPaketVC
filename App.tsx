...
const App: React.FC = () => {
  ...
  // Fix: Implemented exportToCSV to allow users to download their attendance history as a CSV file.
  const exportToCSV = () => {
    ...
    <div className="max-w-md mx-auto min-h-screen px-6 py-10 flex flex-col">
      ...
      <div className="flex justify-between items-end mb-10">
        ...
        <div className="text-right">
          ...
          <div className="flex items-center justify-end space-x-2">
            <div className={`w-1.5 h-1.5 rounded-full ${location && !gpsError ? 'bg-cyan-400' : 'bg-red-500'} animate-pulse`}></div>
            <p className="text-[9px] font-bold text-white/30 uppercase tracking-widest mono">GPS_LINKED</p>
          </div>
        </div>
      </div>

      {/* Modern Tabs */}
      <div className="flex p-1 bg-white/5 backdrop-blur-2xl rounded-2xl border border-white/5 mb-10 overflow-hidden">
        <button 
      ...
      {activeTab === 'attendance' ? (
        ...
        <div className="flex-1 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 overflow-y-auto hide-scrollbar pb-32">
          <div className="flex justify-between items-center px-2">
            ...
            <div className="flex items-center space-x-3">
              <button onClick={exportToCSV} className="p-2 bg-white/5 rounded-lg text-white/40 hover:text-cyan-400 transition-colors">
              ...
              <button 
                ...
                className="p-2 bg-white/5 rounded-lg text-white/40 hover:text-red-500 transition-colors"
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5"></path></svg>
              </button>
            </div>
          </div>
          ...
          {attendanceHistory.length === 0 ? (
            ...
            <div className="space-y-4">
              {attendanceHistory.map((item) => (
                ...
                >
                  <div className="w-16 h-16 rounded-[1.5rem] overflow-hidden bg-slate-800 flex-shrink-0 border border-white/10 shadow-lg">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center space-x-2 mt-2">
                      ...
                      <span className={`text-[8px] font-black uppercase tracking-widest ${item.synced ? 'text-cyan-400/60' : 'text-orange-500/60'}`}>
                        {item.synced ? 'DATABASE_SYNCED' : 'PENDING_UPLOAD'}
                      </span>
                    </div>
                  </div>
                </div>
      ...
      {selectedRecord && (
        <div className="fixed inset-0 z-[100] bg-slate-950/95 backdrop-blur-3xl flex flex-col animate-in fade-in zoom-in-95 duration-300">
          ...
          <div className="flex-1 px-8 flex flex-col space-y-8">
            <div className="flex-1 rounded-[3rem] overflow-hidden border border-white/10 shadow-2xl relative group">
              <img src={selectedRecord.imageUrl} className="w-full h-full object-cover" alt="Verified Presence" />
              <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent"></div>
              <div className="absolute bottom-10 left-10 right-10 space-y-4">
                 <div className="space-y-1">
                    <p className="text-white text-3xl font-black tracking-tighter uppercase">{selectedRecord.studentName}</p>
                    <p className="text-cyan-400 text-sm font-bold tracking-widest uppercase">{selectedRecord.paket} • {selectedRecord.kelas}</p>
                 </div>
                 <div className="flex items-center space-x-4 pt-4 border-t border-white/10 text-[10px] font-bold text-white/50 mono">
                    <span>{selectedRecord.timestamp}</span>
                    <span>•</span>
                    <span className="truncate">{selectedRecord.address}</span>
                 </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 pb-12">
               <button 
                  onClick={() => {
                    const link = document.createElement('a');
                    link.href = selectedRecord.imageUrl;
                    link.download = `ERDI_VER_${selectedRecord.studentName}_${Date.now()}.jpg`;
                    link.click();
                  }}
                  className="py-5 bg-indigo-600 rounded-[2rem] font-black text-[10px] uppercase tracking-widest text-white shadow-xl flex items-center justify-center space-x-3 active:scale-95"
               >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" strokeLinecap="round" strokeLinejoin="round" strokeWidth="3"></path></svg>
                  <span>DOWNLOAD</span>
               </button>
               <button 
                  onClick={() => {
                    if(window.confirm("PERMANENTLY DELETE THIS LOG?")) {
                      deleteRecordLocally(selectedRecord.id).then(() => {
                        setSelectedRecord(null);
                        loadHistory();
                      });
                    }
                  }}
                  className="py-5 bg-white/5 border border-red-500/30 text-red-500 rounded-[2rem] font-black text-[10px] uppercase tracking-widest flex items-center justify-center space-x-3 active:bg-red-500 active:text-white"
               >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5"></path></svg>
                  <span>DELETE</span>
               </button>
